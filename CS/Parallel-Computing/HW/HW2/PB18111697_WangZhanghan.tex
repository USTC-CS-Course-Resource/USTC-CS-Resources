	...
	\begin{algorithmic}[1] %每行显示行号
		\Function {{\sc Sorted-Tree-To-Array}}{T}
			...
			\For {k = 1 to $\ceil{1+\log n}$} \Comment{初始化 level 数组, 自上而下遍历每层, $O(\log n)$}
				...
				\EndFor
			\EndFor
			\For {k = $\log p$ to 1} \Comment{逐层上传统计数量, $O(\log n)$}
				\For {proc i in level k \textbf{par-}} \Comment{初始化}
					\State $size[i] = size[RC_i] + size[LC_i]$
				\EndFor
			\EndFor
			\State
			
			\For {k = 1 to $\log p$} \Comment{逐层上传统计数量, $O(\log n)$}
				\For {proc i in level k \textbf{par-}} \Comment{初始化}
					\If {i 是右节点}
						\State $rank[i] = rank[parent[i]] + 1$
					\ElsIf{i 是左节点}
						\State 
					\EndIf
				\EndFor
...
根据前面的分析知道, \textbf{时间复杂度是 $O(\log n)$ 的}, 所采用的并行计算模型是\textbf{}.
\end{document}
