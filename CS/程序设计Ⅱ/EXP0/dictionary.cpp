...
add word_ex_sen	\t查询单词,(word：操作单词；ex：解释；sen：例句)\n\
...
void dealCommand(entryArray* entries, char* command) {
	...
	if (strstr(command, "help") - command == 0) {
		...
		strcpy(word, strtok(command + 4, "_"));
		if ((strcpy(explain, strtok(NULL, "_"))) == NULL || strcpy(sentence, strtok(NULL, "_")) == NULL) {
			printf("无效命令！\n");
		}
		else {
			strcpy(en->word, word);
		...
		strcpy(word, strtok(command + 5, "_"));
		if (strcpy(item, strtok(NULL, "_")) == NULL || strcpy(data, strtok(NULL, "_")) == NULL) {
			printf("无效命令！\n");
		}
		else if (item[0] == 'w') {
	...
	else if (strstr(command, "all0") - command == 0) {
		printAllEntries(entries, 0);
	}
	else if (strstr(command, "all1") - command == 0) {
		printAllEntries(entries, 1);
	}
	else if (strstr(command, "wordsNum") - command == 0) {
		printf("目前有%d个单词\n", entries->wordsNum);
	}
	else if (strstr(command, "fwordsNum") - command == 0) {
		printf("目前有%d个单词(f)\n", entries->fwordsNum);
	}
	else if (strstr(command, "quit") - command == 0) {
		exit(0);
	}
	else {
		printf("无效命令！\n");
...
entry* searchEntry(entryArray* entries, char* word) {
	...
	end = entries->startLetterIndex[word[0]-'a'];
	//二分查找 
	while (true) {
		middle = (start + end) / 2;
...
void addEntry(entryArray* entries, entry* en) {
	...
	if (en == NULL) {
		...
		en->flocation = temp->flocation + strlen(temp->word) + strlen(temp->explain) + strlen(temp->sentence) + 5;
		...
		//修改处
		entries->startLetterIndex[en->word[0] - 'a']++;
		sortEntryArray(entries);
		printf("增改词条成功\n");
...
void deleteEntry(entryArray* entries, char* word) {
	...
	if (toDelete == (entry*)NULL) {
		...
		fclose(dict);
		//修改处
		entries->startLetterIndex[word[0] - 'a']--;
		printf("删除单词%s成功！\n", word);
	}
