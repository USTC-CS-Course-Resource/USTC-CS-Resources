...
\hline
\end{tabular}
假设性能相同应该有$e_{01}=e_{10}$, $|e_{01}-e_{10}|$服从只有大于0部分的类正态分布, 从而$$\tau_{\chi^2}=\frac{(|e_{01}-e_{10}|-1)^2}{e_{01}+e_{10}}\sim\chi_1^2$$
...
\addplot[no markers, color=blue, domain=-20:20, samples=1000, line width=2] {1/(1+exp(-x))};
\end{axis}
\end{tikzpicture}
\item 以此则有
